...
## Session 2026-09-02 (cont. 3) — Problem 1 tests confirmed green, Problem 1 formally closed
- User confirmed all 16 tests pass (`dotnet test`, `npm test`), `npm audit` clean (0 vulnerabilities) after bumping `vitest` to `^4.1.9` to resolve 5 flagged dev-tooling advisories (esbuild, transitive via vite/vitest — build/test-only, no production exposure).
- `context/index.md` updated: Problem 1 marked COMPLETE.
- User picked **Problem 2: Track Resolution (Spotify Track → YouTube video)** as the next problem, specifically the "track-to-YouTube resolution" piece of the README's stated scope (not yet Download/Acquisition).

## Session 2026-09-02 (cont. 4) — Problem 2 Phase 1 (concept)
- Inspected existing domain files (`Track.cs`, `glossary.md`, `domain-model.md`, `context-map.md`, `file-registry.md`) before discussion, per this skill's setup step.
- Asked and resolved three scoping questions: (1) trigger — **manual per-track AND batch** (not automatic-on-import); (2) candidate selection — **automatic best-match heuristic**, no user-facing candidate picker; (3) where the YouTube call lives — **backend (C#)**, same statelessness pattern as Spotify Integration.
- Presented a fuller concept covering bounded context confirmation, proposed domain model (initially considered a separate `ResolvedTrack` entity), the matching heuristic shape, failure/threshold handling, batch partial-failure behavior, and the YouTube search source choice (Data API vs. yt-dlp).
- User resolved the three remaining open questions: (1) resolution stored as **fields directly on the existing Track** (not a separate entity) — simplifies the Library shape, no join needed client-side; (2) YouTube search via **yt-dlp** (no API quota, accepting fragility — user's own ToS awareness, per open-issues.md item 5); (3) batch partial failure — **never stop early, always return a full resolved/failed summary** (consistent with the silent-fail-but-visible-signal philosophy already established in Problem 1).
- User confirmed the full concept ("све океј") — proceeded to Phase 2 without further changes.

## Session 2026-09-02 (cont. 5) — Problem 2 Phase 2 (implementation)
- Inspected existing backend/frontend structure and file contents (Track.cs, ISpotifyPlaylistReader.cs, ImportPlaylistUseCase.cs, PlaylistDtos.cs/Endpoints.cs, Program.cs, frontend types/api/db/hook/components) before writing new code, to match existing conventions exactly.
- Implemented per the agreed concept: Domain (`Track.ResolveTo`, `YouTubeVideoId`, `TrackResolved`), Application (`IYouTubeTrackSearcher` port, `TrackMatchScorer` pure heuristic, `ResolveTrackUseCase`, `ResolveTracksBatchUseCase`), Infrastructure (`YtDlpTrackSearcher` — shells out to the yt-dlp CLI via `Process`, `YouTubeOptions`, DI extension), API (`TrackResolutionDtos.cs`, `TrackResolutionEndpoints.cs`, `Program.cs`/`appsettings.json` wiring), Frontend (`tracks/api.ts`, `useTrackResolution.ts`, `TrackRow`, `BatchResolutionSummaryToast`, `PlaylistCard` extended to show a track list, `App.tsx`/`App.css` wiring).
- **User-caught build failure, corrected in-session:** `npm run build` failed with `TS2339: Property 'env' does not exist on type 'ImportMeta'` in both the pre-existing `playlists/api.ts` and the new `tracks/api.ts`. Root cause: `frontend/src/vite-env.d.ts` (the standard Vite triple-slash `/// <reference types="vite/client" />` file) was never created during Problem 1 — this had gone undetected because Problem 1's Phase 3 verification only ran `npm test` (Vitest's own typing doesn't need this reference), never `npm run build`'s full `tsc -b` check. Fixed by adding the missing file — a one-line, surgical fix, not part of Problem 2's actual scope but corrected here since it blocked verifying Problem 2's own build.
- User confirmed both `npm run build` and `dotnet build` pass after the fix. Phase 2 for Problem 2 complete.
- **Next session:** Phase 3 (test scenarios) for Problem 2.

## Session 2026-09-02 (cont. 6) — Problem 2 Phase 3 (validation), Problem 2 closed
- Proposed 18 test scenarios (13 backend, 5 frontend) covering: `TrackMatchScorer` heuristic behavior (exact match, unrelated title, neutral-vs-penalized unknown duration, best-of-multiple, empty list, far-duration falloff), `ResolveTrackUseCase` success/no-confident-match/no-candidates paths, `ResolveTracksBatchUseCase` partial-failure and all-fail paths, `Track.ResolveTo` invariants (field-setting, event, overwrite-on-retry, confidence-range validation), and `useTrackResolution` hook success/no-match/network-failure/batch-summary/independent-clear paths. User confirmed to proceed with test code.
- Trimmed to the highest-value subset when writing: 4 Domain tests, 6+3+2 = 11 Application tests, 5 frontend tests — 20 total, covering the same scenarios more compactly (e.g. one parameterized `[Theory]` for the two confidence-out-of-range cases instead of two separate `[Fact]`s).
- **User-reported test failure, corrected in-session:** the frontend "clearError/clearBatchSummary reset independently" test failed — `expected null not to be null` on `result.current.error`. Root cause was the test itself, not the hook: it called `doResolve` (sets `error`) and then `doResolveBatch` (which also calls `setError(null)` on entry per the hook's existing pattern, wiping the just-set `error` before the assertion ran). Fixed by reordering the test to call `doResolveBatch` first (assert `lastBatchSummary`), then `doResolve` (assert `error`, and confirm `lastBatchSummary` survives since `doResolve` doesn't touch it), before testing the two clear functions independently.
- User confirmed all tests pass: backend `dotnet test` (21 total, 11 from Problem 1 + 10 new), frontend `npm test` (10 total, 5 from Problem 1 + 5 new).
- Updated all context files: `index.md` (Problem 2 COMPLETE, Problem 3 ready), `domain-model.md` (Problem 2 section), `context-map.md` (Track Resolution moved to confirmed), `glossary.md` (Problem 2 confirmed/candidate terms), `file-registry.md` (full file list for Problem 2 + tests), `review-log.md` (this entry).
- Problem 2 is fully done (all 3 phases). **Next session:** ready to pick Problem 3 — likely candidates per context-map.md's remaining deferred contexts: Download/Acquisition (fetch audio for a resolved Track) or Playback/Offline.
